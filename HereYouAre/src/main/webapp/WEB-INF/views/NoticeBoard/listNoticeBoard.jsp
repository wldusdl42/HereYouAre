...
// 	var nBlockStart = ${nBlockStart};
// 	var nBlockEnd = ${nBlockEnd};
// 	var maxPageNum = ${maxPageNum};
//  var page_id
// 	console.log(nBlockStart);
// 	console.log(nBlockEnd);
	       ...
	       <div class="list_wrapper">
	         <ul>
		         <c:forEach items="${list}" var="noticeBoard">
		           <div class="line">
		             <a href="getNoticeBoardContents?bId=${noticeBoard.id}" style="color: #1e1e1e"><li class="line_cont">${noticeBoard.title}</li></a><li class="line_time" >${noticeBoard.write_time}</li>
		           </div>
		          </c:forEach>
	         </ul>
	         <div class="paging">
    			<ul class="paginationBox">
    				<c:if test="${page_id ne 1}">
    					<a class="page-link" href="/are/1"><i class="fa fa-angle-double-left faPointer"></i></a>
    				</c:if>
	    			<c:if test="${page_id ne 1}">
	    				<a class="page-link" href="/are/${page_id-1}"><i class="fa fa-angle-left faPointer"></i></a>
	    			</c:if>
						  <c:forEach var="cnt" begin="${nBlockStart}" end="${nBlockEnd}">
						  <c:choose>
						  	<c:when test="${page_id == cnt}">
						  		<li class="page"><a class="page-link_select" href="/are/${cnt}">${cnt}</a></li>
						  	</c:when>
						  	<c:otherwise>
						  		<li class="page"><a class="page-link" href="/are/${cnt}">${cnt}</a></li>
						  	</c:otherwise>
						  	</c:choose>
						  </c:forEach>
					<c:if test="${page_id ne maxPageNum}">
						<a class="page-link" href="/are/${page_id+1}"><i class="fa fa-angle-right faPointer"></i></a>
					</c:if>
					<c:if test="${page_id ne maxPageNum}">
						<a class="page-link" href="/are/${maxPageNum}"><i class="fa fa-angle-double-right faPointer"></i></a>
					</c:if>
				</ul>

					<form action="/are/search/1" method="GET" name="search">
						<input type="hidden" name="page_id" value="1">
						<input type="hidden" name="keyword_remain" value="${keyword_remain}">
