...
function setEmail(eVal) {
	var e1 = document.all("e1") //User input
...
function checkValue() {
  ...
  //비밀번호 확인 검사
    if($("#password").val() != $("#passwordChk").val()){
	 ...
	 // 비밀번호 유효성 확인 (문자, 숫자, 특수문자, 6자리~16자리)
	 var password = document.joinUser.password;
...
function ShowPostcode(){
	new daum.Postcode({
        oncomplete: function(data) {
            ...
            //내려오는 변수가 값이 없는 경우엔 공백('')값을 가지므로 이를 참고하여 분기함
            ...
            if(data.userSelectedType==='R'){
            	...
            	if(data.buildingName!=='') {//건물명이 있을 경우 추가
            		extraAddr+=(extraAddr!==''?', ' +data.buildingName : data.buildingName);
            	}
            	fullAddr += (extraAddr!==''?' ('+extraAddr+')':'');//조합형주소의 유무에 따라 양쪽 괄호를 추가하여 최종 주소를 만듬
            }
            document.getElementById('zip_code').value = data.zonecode;//우편번호와 주소정보를 해당 필드에 넣음
            document.getElementById('a1').value = fullAddr;
            document.getElementById('a2').focus();//커서를 상세주소 필드로 이동
        }
    }).open();
}
